...
class BaseDataset:
    ...
    def format(self, fewshot: int = 0): # The result of this function is the definition of self.dataset
        def _format(
            example: Dict,
            use_answer: bool = False,
            # input_template_func: Callable = None, 
            input_template_func: Callable = lambda ques: f'Question: {ques}\nAnswer:'
            # The rewritten code relies on this form
        ):
            q = example['question']
            if 'cot' in example:
            ...
            if use_answer:
                ...
                # (e.g. WikiMultiHopQA) lambda self, cot, ans: f'{cot} So the answer is {ans}.'
            return query

        # demo
        # As a note in the fresh-shot
        demo = [{
            'question': self.examplars[i]['question'],
            'case': _format(self.examplars[i], use_answer=True, input_template_func=self.demo_input_template), # Packed questions and answers.
            'ctxs': self.examplars[i]['ctxs'] if 'ctxs' in self.examplars[i] else [] # No ctxs in WikiMultiHopQA!
        } for i in range(fewshot)] if fewshot else []

        def _format_for_dataset(example):
            # case
            # Meaning: (especially of the police) a case of investigation, a case of inquiry; a case to be adjudicated
            # That is, the question to be answered (and therefore no answer given)
            case = _format(example, use_answer=False, input_template_func=self.test_input_template)
            # ctx
            example['demo'] = demo
            example['case'] = case 
            # Here in the implementation example[“case”] will be unimportant, the important thing will be example[“question”]
            return example
...
class StrategyQA(BaseDataset):
    ...
    def __init__(self, data_path: str):
        ...
        dataset = []
        with open(os.path.join(data_path, "strategyqa_train.json"), "r", encoding='utf-8') as fin:
            dataset_1 = json.load(fin)
        with open(os.path.join(data_path, "strategyqa_train_paragraphs.json"), "r", encoding='utf-8') as fin:
...
class WikiMultiHopQA(BaseDataset):
    examplars: List[Dict] = [
        {
            ...
            'cot': "The film Hypocrite was directed by Miguel Morayta. Miguel Morayta died on 19 June 2013.", # cot is CoT, Chain of Thought
            'answer': "19 June 2013",
        },
        {
    ...
    def __init__(self, data_path: str): 
        ...
        self.dataset = Dataset.from_list(dataset) # Getting the Dataset type
        self.init_id_aliases(data_path)
        
    @classmethod
    def init_id_aliases(cls, data_path): # alias：nickname
...
class HotpotQA(BaseDataset):
    ...
    def __init__(self, data_path: str):
        ...
        dataset = []
        with open(os.path.join(data_path, 'hotpotqa-dev.json'), "r", encoding='utf-8') as fin:
            js = json.load(fin)
            for example in tqdm(js):
                qid = example["_id"]
                question = example["question"]
